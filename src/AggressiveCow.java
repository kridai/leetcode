...
public class AggressiveCow {
  ...
  class Solution {
    public int solve(int[] A, int B)  {
      ...
      Arrays.sort(A);
      long val = binarySearchAnswerSpace(0, A[A.length -1], A, B-1);
      return (int) val;
    }
    public int binarySearchAnswerSpace(int start, int end, int sumArray[], int A) {
      if(start >= end-1) {
        if(isValuePossible(end,sumArray, A, 1, end+sumArray[0])) {
          return end;
        }
        return start;
      }
      int mid = start + (end-start)/2;
      if( isValuePossible(mid,sumArray, A, 1, mid+sumArray[0])) {
        return binarySearchAnswerSpace(mid, end, sumArray, A);
      }
      else {

        return binarySearchAnswerSpace(start, mid-1, sumArray, A);
      }
    }

    public boolean isValuePossible(int incVal, int sumArray[], int A, int start, int val) {
      if(A == 0 ){
        return true;
      }
      ...
      if(index == (sumArray.length - 1) && A != 1) {
        return false;
      }

      int indexValue = sumArray[index];

      return isValuePossible(incVal , sumArray, A-1, index +1, indexValue+incVal);
    }

    public int binarySearch(int start, int end, int sumArray[], int val) {
      if(start >= end-1) {
      ...
      if(sumArray[mid] < val) {
        return binarySearch(mid+1, end, sumArray, val);
      }
      else {
